...
async def test_pasv_connection_ports(
    pair_factory, Server, unused_tcp_port_factory
):
...
async def test_pasv_connection_port_reused(
    pair_factory, Server, unused_tcp_port
):
    ...
    async with pair_factory(None, s) as pair:
        ...
        await pair.client.connect(
            pair.server.server_host, pair.server.server_port
        )
...
async def test_pasv_connection_no_free_port(
    pair_factory, Server, expect_codes_in_exception, method
):
    s = Server(data_ports=[])
...
async def test_pasv_connection_busy_port2(
    pair_factory, Server, unused_tcp_port_factory, expect_codes_in_exception
):
...
async def test_long_login_sequence_fail(
    pair_factory, expect_codes_in_exception
):
    ...
    factory = pair_factory(
        logged=False, server_factory=CustomServer, do_quit=False
    )
